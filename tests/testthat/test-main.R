testthat::test_that("run_mac delegates to broker_fetch and returns result", {
  ...
  out <- with_temp_bindings(
    ...
    run_mac(req, secrets_path = "config/x.yaml")
  )
  testthat::expect_equal(out$connector, "mock")
  testthat::expect_equal(out$request$source, "world_bank")
})

testthat::test_that("run_mac exports when output format and file are set", {
  req <- list(
    source = "world_bank",
    params = list(),
...
testthat::test_that("run_mac does not export when output is absent or file is blank", {
  ...
  with_temp_bindings(
    ...
    run_mac(req_no_output, secrets_path = "config/x.yaml")
  )
  ...
  with_temp_bindings(
    ...
    run_mac(req_blank_file, secrets_path = "config/x.yaml")
  )

  testthat::expect_false(calls$export)
})

testthat::test_that("run_mac reads request file path then dispatches", {
  out <- with_temp_bindings(
    list(
    ...
    run_mac("request.yaml", secrets_path = "config/s.yaml")
  )
  testthat::expect_equal(out$connector, "world_bank")
})
